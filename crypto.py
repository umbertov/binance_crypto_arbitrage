...
class Crypto:
    ...
    def __init__(self):
        self.init_ccxt()
        self.bot = None  # telegram.Bot(token=secrets.TELEGRAM)
    ...
    def get_price_cache(self, exchange, asset1, asset2):
        for item in self.cache_prices:
            if (
                ...
                and item["asset1"] == asset1
                and item["asset2"] == asset2
            ):
                return item["ticker"]
        return None
    ...
    def get_price(self, exchange, asset1, asset2, mode="average"):
        ...
        try:
            ...
            self.log(
                "Error while fetching price for {}/{}: {}".format(
                    asset1, asset2, str(e)
                )
            )
            return None
    ...
    def get_order_book(self, exchange, asset1, asset2, mode="bids"):
        ...
        try:
            ...
            self.log(
                "Error while fetching order book for {}/{}: {}".format(
                    asset1, asset2, str(e)
                )
            )
            return None
    ...
    def is_open_order(self, exchange, asset1, asset2):
        try:
            ...
            self.log(
                "Error while fetching open orders for {}/{}: {}".format(
                    asset1, asset2, str(e)
                )
            )
            return None
    ...
    def cancel_orders(self, exchange, asset1, asset2):
        for _ in range(5):
            try:
                ...
                self.log(
                    "Error while canceling orders for {}/{}: {}. Retrying.".format(
                        asset1, asset2, str(e)
                    )
                )
    ...
    def estimate_arbitrage_forward(self, exchange, asset):
        try:
            ...
            if not alt_BTC or not alt_ETH:
                self.log(
                    "Less than 3 orders for {} on {}, skipping.".format(
                        asset, str(exchange)
                    )
                )
                return -100
            ...
            step3 = (
                step2 / self.get_price(exchange, "ETH", "BTC", mode="ask")
            ) * self.get_fees(exchange, "buy")
            return (step3 - 1) * 100
    ...
    def estimate_arbitrage_backward(self, exchange, asset):
        try:
            ...
            if not alt_BTC or not alt_ETH:
                ...
                return -100
            step1 = (
                self.get_price(exchange, "ETH", "BTC", mode="bid")
            ) * self.get_fees(exchange, "sell")
    ...
    def buy(
        ...
        try:
            if amount_percentage:
                asset2_available = (
                    self.get_balance(exchange, asset2) * amount_percentage
                )
                amount = asset2_available / self.get_price(
                    exchange, asset1, asset2, mode="ask"
                )
            self.log(
                "Buying {:.6} {} with {} on {}.".format(
            ...
            if not limit:
                ...
                exchange.createMarketBuyOrder("{}/{}".format(asset1, asset2), amount)
                return True
            else:
                ...
                if timeout:
                    ...
                    if result == Crypto.ORDER_NOT_FILLED:
                        if self.cancel_orders(exchange, asset1, asset2):
                            self.log(
                                "Canceled limit order for {}/{} after timeout.".format(
                                    asset1, asset2
                                )
                            )
                        return False
                        ...
                        while result == Crypto.ORDER_IN_PROGRESS:
                            ...
                            if n >= config.WAIT_TIMES_WHEN_FILLED:
                                self.log(
                                    "Order cannot be terminated, selling {} to {}.".format(
                                        asset1, asset2
                                    )
                                )
                                ...
                                return False
                            self.log(
                                "Order for {}/{} is in progress, waiting...".format(
                                    asset1, asset2
                                )
                            )
                            time.sleep(timeout)
    ...
    def sell(
        ...
        try:
            ...
            if not limit:
                ...
                exchange.createMarketSellOrder("{}/{}".format(asset1, asset2), amount)
                return True
            else:
                ...
                if timeout:
                    ...
                    if result == Crypto.ORDER_NOT_FILLED:
                        ...
                        while result == Crypto.ORDER_IN_PROGRESS:
                            ...
                            if n >= config.WAIT_TIMES_WHEN_FILLED:
                                self.log(
                                    "Order cannot be terminated, buying {} with {}.".format(
                                        asset1, asset2
                                    )
                                )
    ...
    def summarize_arbitrage(self, exchange, balance_before, asset):
        ...
        self.log(
            "➡️ Arbitrage {:5} on {:10}, diff: {:8.6f}ETH ({:.2f} EUR), balance: {:7.6f}ETH ({:.2f} EUR), {:.2f}%".format(
                asset,
                str(exchange),
                diff,
                diff_eur,
    ...
    def run_arbitrage_forward(self, exchange, asset):
        ...
        if not result1:
            self.log(
                "❌ Failed to convert {} to ETH, canceling arbitrage.".format(asset),
                mode="notification",
            )
            return
        ...
        if not result2:
            self.log(
                "❌ Failed to convert {} to BTC, canceling arbitrage. Will convert back {} to ETH.".format(
                    asset, asset
                ),
                mode="notification",
            )
    ...
    def run_arbitrage_backward(self, exchange, asset):
        ...
        if not result1:
            self.log(
                "❌ Failed to convert BTC to {}, canceling arbitrage. Will convert BTC to ETH.".format(
                    asset
                ),
                mode="notification",
            )
        ...
        if not result2:
            self.log(
                "❌ Failed to convert {} to ETH, canceling arbitrage. Forcing convertion from {} to ETH.".format(
                    asset, asset
                ),
                mode="notification",
            )
    ...
    def best_buy(self, exchange, asset1, asset2, amount_percentage):
        ...
        for price in orderbook[: config.MAX_ORDERBOOK_TRIES]:
            self.log(
                "Trying to buy {} with {} @{:.8f}.".format(asset1, asset2, price[0])
            )
            result = self.buy(
                exchange,
                asset1,
                ...
                limit=price[0],
                timeout=config.WAIT_LIMIT_ORDER,
            )
            if result:
                self.log(
                    "✅ Bought {} with {} @{:.8f}.".format(asset1, asset2, price[0])
                )
                return True
            else:
                self.log(
                    "⏳ Failed to buy {} with {} at {:.8f}.".format(
                        asset1, asset2, price[0]
                    )
                )
    ...
    def best_sell(self, exchange, asset1, asset2, amount_percentage):
        ...
        for price in orderbook[: config.MAX_ORDERBOOK_TRIES]:
            self.log(
                "Trying to sell {} to {} @{:.8f}.".format(asset1, asset2, price[0])
            )
            result = self.sell(
                exchange,
                asset1,
            ...
            if result:
                ...
                self.log(
                    "⏳ Failed to sell {} to {} at {:.8f}.".format(
                        asset1, asset2, price[0]
                    )
                )
