    ...
    if delta_forward > config.THRESHOLD:
        ...
        print(
            ...
            "Found opportunity for {:5} @{:.4f} on {}\n".format(
                alt, delta_forward, str(exchange)
            ),
        )
        crypto.log(
            "Found opportunity for {:5} @{:.4f} on {}".format(
                alt, delta_forward, str(exchange)
            ),
            mode="log",
        )
        ...
        print(
            "\n",
            "Found opportunity for {:5} @{:.4f} on {}\n".format(
                alt, delta_backward, str(exchange)
            ),
        )
        crypto.log(
            "Found opportunity for {:5} @{:.4f} on {}".format(
        ...
        for i in range(0, len(alts), thread_number):
            ...
            try:
                for asset in alts_batch:
                    ...
                    threads.append(
                        threading.Thread(
                            target=process_asset, args=(crypto, exchange, asset)
                        )
                    )
                    threads[-1].start()
