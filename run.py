...
def process_asset(crypto, exchange, alt):
        ...
        if (delta_forward > config.THRESHOLD):
                print(victory_msg)
                print("\n", "Found opportunity for {:5} @{:.4f} on {}\n".format(alt, delta_forward, str(exchange)))
                crypto.log("Found opportunity for {:5} @{:.4f} on {}".format(alt, delta_forward, str(exchange)), mode="log")
                #crypto.run_arbitrage_forward(exchange, alt)
        elif (delta_backward > config.THRESHOLD):
                print(victory_msg)
                print("\n", "Found opportunity for {:5} @{:.4f} on {}\n".format(alt, delta_backward, str(exchange)))
                crypto.log("Found opportunity for {:5} @{:.4f} on {}".format(alt, delta_backward, str(exchange)), mode="log")
